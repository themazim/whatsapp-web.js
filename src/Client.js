...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        const INTRO_IMG_SELECTOR = '[data-icon="search"],[data-icon="chat"],[data-icon="intro-md-beta-logo-dark"],[data-icon="intro-md-beta-logo-light"]';
        ...
        await page.exposeFunction('onIncomingCall', (call) => {
            ...
            console.log(window.Store.Call);

            for (const key in window.Store.Call) {
                if (typeof window.Store.Call[key] === 'function') {
                    console.log(key);
                }
            }
                ...
                if (res.ok() && res.url() === WhatsWebURL) {
                    await webCache.persist(
                        await res.text().catch(error => {
                            console.log('Failed to request WebVersion:');
                            console.log(JSON.stringify(error));
                        })
                    ).catch(error => {
                        console.log('Failed to persist WebVersion:');
                        console.log(JSON.stringify(error));
                    });
                }
            });
    ...
    async searchChannels(searchOptions = {}) {
        return await this.pupPage.evaluate(async (searchOptions, _sortField, _sortOrder, _viewType) => {
            ...
            limit !== 50 && window.injectToFunction({
                module: 'getNewsletterDirectoryPageSize',
                index: 0,
                function: 'getNewsletterDirectoryPageSize'
            }, () => limit);
